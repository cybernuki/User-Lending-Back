...
fundsLib.finishFund = async (fund_id) => {
  ...
  let fund = await Funds.findOne({
    attributes: ['status'],
    where: {
      id: fund_id
    }
  });
  if (fund.status != FUNDS_STATUTS['gathering']) return null;

  let change = await fundsLib.changeStatus(fund_id, 'done');
  if (!change) return false;
  //change next waiting fund to gathering status
  let next = await fundsLib.getNextWaiting();
  if (next) {
    let change = await fundsLib.changeStatus(next.dataValues.id, 'gathering');
    if (!change) return false;
  }
  return true;
};
...
fundsLib.updateAmount = async (fund_id, newFund) => {
  try {
    let result = await Funds.increment({ amount: newFund }, { where: { id: fund_id } })
