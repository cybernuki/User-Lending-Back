const { Aspirants } = require('../models'),
  { search } = require('./lib.funds'),
  axios = require('axios').default;

const aspirantLib = { name: 'aspirantsLib' }

aspirantLib.getAll = async () => {
  const aspirants = await Aspirants.findAll({})
  return aspirants
}

// get by id
aspirantLib.search = async (aspirantId) => {
  const aspirant = await Aspirants.findOne({
    where: {
      storeKeeperId: aspirantId
    }
  });
  const fund = await search(aspirantId);
  return { aspirant, fund };
}


// Register new aspirant
aspirantLib.create = async (aspirant_id, email) => {
  try {
    ...
    if (!error.errors) return console.error(error);
    else if (error.errors[0].type == 'unique violation') return aspirantLib.search(aspirant_id)
  }
}
 ...
 */
aspirantLib.verifyEmail = async (email) => {
  try {
 ...
 */
aspirantLib.getToken = async (email, password) => {
  const data = {
 ...
 * @param {*} token 
 */
aspirantLib.getStoreKeeperId = async (token) => {
  const options = {
    headers: {
      'Content-Type': 'application/json',
